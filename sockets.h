...
public:
	...
	int SendInt(int);									//отправить число
	int GetInt(int&);									//получить число и записать его в переданный по ссылке аргумент
	void SetBlock(int flags);							//установить флаги на сокет
	Socket& GetPushSocket();							//получить ссылку на сокет
