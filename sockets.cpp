...
int PushPullSocket::GetInt(int& num) {
	door.lock();													//блокируем мьютекс
	zmq_msg_t reply;												//создаем zmq-сообщение
	zmq_msg_init_size(&reply, sizeof(int));							//иницилизируем его с конкректным размером
	int res;
	if ((res = zmq_msg_recv(&reply, pull.socket, flags)) != -1) {		//принимаем из pull сокета
		memcpy(&num, zmq_msg_data(&reply), zmq_msg_size(&reply));		//копируем в num число, которое считали
		zmq_msg_close(&reply);										//закрываем zmq-сообщение
	}
	door.unlock();													//отпускаем мьютекс
	return res;														//возвращаем кол-во считанных байт
}
